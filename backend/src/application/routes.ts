...
// 提交调解申请
router.post('/', [auth, roleAuth(['personal', 'company', 'tenant_admin', 'superadmin'])], createApplication);

// 获取当前街道值班调解员
router.get('/duty-mediator', [auth, roleAuth(['personal', 'company', 'tenant_admin', 'superadmin'])], getCurrentDutyMediator);

// 提交咨询
router.post('/consultation', [auth, roleAuth(['personal', 'company', 'tenant_admin', 'superadmin'])], createConsultation);
